...
class TransformerEncoder(nn.Module):
    def __init__(self, seq_length=60, embed_dim=300, depth=8, num_heads=6, head_dim=0,
        ...
        self.pos_embed = nn.Parameter(torch.zeros(1, 1 + seq_length, embed_dim))
    ...
    def forward_features(self, x, mask=None): # [B, L, F]
        cls_token = self.cls_token.expand(x.shape[0], -1, -1) # [1, 1, F] -> [B, 1, F]
        x = torch.cat((cls_token, x), dim=1)  # [B, L + 1, F]
        if mask is not None:
            cls_mask = torch.ones(x.shape[0], 1).to(mask.device)
